...
\begin{align*}
    & \text{Si on a besoin de }\boxed{\P_k} \text{ alors \textbf{on se ramène à une récurrence simple.}}\\
    & \text{Si on a besoin de }\boxed{\P_{k-1}} \text{ alors \textbf{on se ramène à une récurrence double.}}\\
    & \text{Si on a besoin de }\boxed{\P_{k-2}} \text{ alors \textbf{on se ramène à une récurrence triple.}}\\
    & \ldots\ldots\ldots
\end{align*}
Et donc, les initialisations et l'intervalle de notre hypothèse de récurrence changera en conséquence et on remarque alors que si le plus petit rang nécessaire est \(\P_{k-p}\), alors on se ramène nécessairement à une \textbf{récurrence à \(p\) pas}, avec \(p\) initialisations et l'hypothèse de récurrence qui commence à \(\alpha + p\).\<
...
Intuitivement, les bijections sont exactement \textbf{les éléments inversibles à gauche et à droite} par une même application.\<
