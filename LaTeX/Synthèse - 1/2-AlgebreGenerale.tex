...
On considère tout d'abord le groupe quotient \( \Z /n\Z \), on peut alors remarque que les éléments de ce groupe sont exactement \textbf{les classes de restes possibles par la division euclidienne par \( n \)}.  Par exemple le groupe \( \Z/6\Z\) représente le fait que \( 2 \equiv 8 \equiv 14 [6]\).
...
\]
Donc en particulier \( [a] + \ldots + [a] = 1 \) dans \( \Z /n\Z \) et par la suite, \( a \) engendre tout le groupe. Il y a donc \( \varphi(n) \) générateurs de ce groupe.
