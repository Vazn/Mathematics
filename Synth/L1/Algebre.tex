\chapter*{\chapterstyle{Corps des Complexes}} % 99% Fini
...
On appelle \textbf{module} de \(z\) le \textbf{prolongement} de la fonction valeur absolue à \(\C\), c'est donc une \textbf{métrique} et on la définit telle que {:}
\[
    \rho = \sqrt{a^2 + b^2} = \sqrt{z\overline{z}} 
\]

\subsection*{\subsecstyle{Forme trigonométrique {:}}}
On montre alors qu'il existe un unique angle \(\theta\) (modulo \(2\pi\)) qu'on appelle \textbf{argument} de \(z\) tel que:
\[
    z = \rho(\cos\theta+i\sin\theta)   
\]

\subsection*{\subsecstyle{Forme exponentielle {:}}}
De même on appelle \textbf{forme exponentielle} de \(z\) l'expression:
\[
    z = \rho e^{i\theta} 
\]

Les propriétés de l'exponentielle nous permettent aussi d'obtenir:
\multiparbox{-0.5em}{0.4}{
    \begin{align*}
...
\]
C'est un morphisme \textbf{additif} et \textbf{multiplicatif}. Graphiquement, le conjugué d'un nombre complexe est \textbf{le symétrique} de \(z\) par rapport à l'axe réel.\<

On a alors les formules suivantes {:}
\multiparbox{0.5em}{0.37}{
     \[
         \begin{aligned}
...
\begin{center}
    \textit{Les différentes puissances d'un nombre complexe (de module 1) s'interprétent alors comme des points situées à equidistance sur un cercle.}
\end{center}
Graphiquement:
\begin{center}
...
\chapter*{\chapterstyle{Anneau des Polynômes}} % 75% Fini
...
\begin{center}
    \textbox{
        Tout polynôme de degré supérieur à un admet au moins une racine dans \(\C\).
    }
\end{center}
...
Par contre dans \(\R[X]\), il existe des polynômes de degré 2 irréductibles\footnote[2]{Ceux dont le discriminant est négatif}.\<
...
\chapter*{\chapterstyle{Espaces Vectoriels}} % 99% Fini
...
\chapter*{\chapterstyle{Applications Linéaires}} % Fini 99%
...
\chapter*{\chapterstyle{Théorie de la dimension}} % Fini 99%
...
\chapter*{\chapterstyle{Espace des matrices}} % Fini 99%
...
\chapter*{\chapterstyle{Déterminant}} % Fini 99%
