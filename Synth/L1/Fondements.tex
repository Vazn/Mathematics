...
Si \(F \subseteq G\), alors on définit \textbf{la composée} \(g \circ f\) par la fonction \(h: x \in E \longmapsto g(f(x)) \in H\) et on a alors la propriété suivante:
\customBox{width=15cm, left*=0mm, right*=0mm}{
    L'ensemble des fonctions muni de la composition est alors muni \textbf{d'une structure d'anneau}.
}
