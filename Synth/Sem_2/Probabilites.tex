...
En d'autres termes:
\begin{center}
   \textit{L'image réciproque des images de \(X\) sont des évenements.}
\end{center}
...
Soit \((\Omega, \Pow(\Omega), \mathbb{P})\) un espace probabilisé fini et \(X\) une variable aléatoire réelle finie, on note \(\mathbb{P}_X\) la loi de \(X\). \<
...
L'espérance possède plusieurs propriétés remarquables, l'espérance d'une constante est une constante, elle est \textbf{linéaire et croissante}\footnote{Croissante pour l'ordre des variables aléatoires, ie \(X \leq Y \Longleftrightarrow \forall \omega \in \Omega \; ; \; X(\omega) \leq Y(\omega)\)}.\+
...
\]
Qui implique alors la considération suivante:
\begin{center}
   \textit{L'espérance de la composée dépent uniquement de la loi de \(X\)}
\end{center}
...
\[
   \mathbox{
      \variance{X} = \expectancy{(X - \expectancy{X})^2}
   }   
\]
Alors la variance est \textbf{positive} (ou nulle si \(X\) est constante presque partout).\<
\pagebreak
...
Mais en général, la variance \textbf{n'est pas additive}, c'est néanmoins le cas quand les deux variables aléatoires sont \textbf{indépendantes}.\<

On peut alors définir \textbf{l'écart-type} de la variable \(X\) qui est:
\[
   \mathbox{
...
Il est intéressant de considérer les différents indicateurs de position et de dispersion des lois usuelles\footnote{Pour la loi uniforme on considère \(X\) à valeurs dans \(\inticc{1}{n}\)}
