...
La limite étant définie composantes par composantes, la notion de continuité s'étend naturellement composantes par composantes et on dira alors (comme dans le cas réel) que \(f\) est continue si et seulement si:
\[
...
\chapter*{\chapterstyle{VII --- Continuité}}
...
On s'intéresse alors à ce qu'on peut déduire de l'annulation d'une seule des dérivées partielles, et en se ramenant à l'étude des application partielles on peut alors montrer que si \(f : \mathcal{U} \rightarrow \R\) différentiable sur \(\mathcal{U}\) ouvert et \textbf{convexe}\footnote[2]{\textbf{ATTENTION:} Ce résultat est faux sur un domaine juste connexe. Prendre la fonction qui définie sur \(\R^2\) privée des axes qui vaut \(0\) sur \(\{ (x, y), xy < 0\}\) et \(x^2\) ailleurs.} et que \(\partialD{f}{x_i}\) est \textbf{nulle} sur \(\mathcal{U}\), alors il existe une fonction \(g\) telle que:
\[
...
\chapter*{\chapterstyle{VII --- Introduction aux EDP}}
\addcontentsline{toc}{section}{Introduction aux EDP} 
...
\addcontentsline{toc}{section}{Intégrale Curviligne} 

Dans cette partie, nous allons généraliser l'intégrale de Riemann, c'est à dire l'intégrale d'une fonction le long d'un segment, aux \textbf{champs vectoriels}, avec en tête l'idée suivante:
\begin{center}
   \textit{On veut pouvoir intégrer un champ vectoriel le long d'une courbe lisse donnée.}
\end{center}
Nous verrons alors que l'approche sur les champs vectoriels sera une vraie généralisation, qui s'appliquera alors aux champs scalaires et donc aux intégrales réelles.\<

Dans toute la suite, on considèrera un champ de scalaires \(f : \R^n \rightarrow \R^p\) ainsi qu'un \textbf{arc orienté} \(\Gamma\) paramétré par une fonction \(\gamma : t \mapsto \gamma(t)\) définie sur \(\icc{a}{b}\) et de classe \(\mathcal{C}^1\). On rapelle à toutes fins utiles que l'abcisse curviligne de \(\Gamma\) d'origine \(a\) est donnée par:
\[
   s(t) = \int_{a}^{t} \vectNorm{\gamma'(x)} \d x   
\] 

\subsection*{\subsecstyle{Compléments sur les différentielles{:}}}
On considère tout d'abord une famille \((\phi_1, \ldots, \phi_p)\) de \textbf{formes linéaires} sur \(\R^n\), on aimerait alors donner un sens à l'expression suivante:
\[
   (\phi_1, \ldots, \phi_p)   
\]
On définit alors:
\[
   \begin{aligned}
      (\phi_1, \ldots, \phi_p) : \R^n &\longrightarrow \R^p
      x &\longmapsto (\phi_1(x), \ldots, \phi_p(x)) = \sum_{i=1}^{p} \phi_i(x)e_i
   \end{aligned}
\]
On considère la différentielle du champ vectoriel \(f\), alors elle est donnée en composantes par:
\[
   df_a = (df_{(1, a)}, \ldots, df_{(p, a)})   
\]
Donc en particulier, dans le cas de la courbe paramétrée \(\Gamma\), on a:
\[
   d\gamma_t = (\gamma_1'(t) dt, \ldots, \gamma_n'(t) dt) = (\gamma_1'(t), \ldots, \gamma_n'(t))dt  
\]