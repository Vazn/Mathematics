...
\chapter*{\chapterstyle{VI --- Suites numériques}} % 95% Fini
...
\chapter*{\chapterstyle{VI --- Topologie élémentaire de \(\R\)}} % 95% Fini
...
\chapter*{\chapterstyle{VI --- Limites}} % 99% Fini
...
\]
On applique alors le critère de Cauchy à la série et on obtient alors que la série converge si:
\[
   \lim_{n \rightarrow +\infty}|x|\sqrt[n]{|a_n|} < 1
\]
Si la limite du \textbf{critère de Cauchy} existe\footnote[2]{Avec la convention que si la limite est nulle, le rayon de convergence est infini.}, on a alors que:
\[
   R := \frac{1}{\lim_{n \rightarrow +\infty}\sqrt[n]{|a_n|}}   
\]
Si la limite du \textbf{critère de d'Alembert} existe, alors on a:
\[
   R = \lim_{n \rightarrow +\infty}\frac{|a_n|}{|a_{n+1}|}
\]
On a alors la propriété de régularité trés forte suivante:
\begin{center}
...
On définit pour se faire les \textbf{polynômes trigonométriques} à coefficients complexes\footnote[1]{On peut alors les définir algébriquement comme étant \(\C[\cos(x), \sin(x)]\) et montrer que ces polynômes on bien une structure d'anneau (et même d'algèbre) de manière analogue aux polynômes usuels.}, qui sont les fonctions de la forme suivante:
\[
   f(x) = \sum_{k=0}^{n} a_kcos(k\omega x) + b_ksin(k\omega x)    
\]
On a ici \(a_n, b_n\) deux suites de coefficients complexes et \(\omega\) un réel. Plus généralement, gràce aux formules d'Euler, on peut montrer que tout polynôme trigonométrique peut s'écrire sous forme exponentielle par:
\[
   S = \sum_{k = 0}^{n} c_k e^{ik\omega x} + c_{-k}e^{-ik\omega x} \underset{not.}{=} \sum_{k=-n}^{n} e^{ik\omega x}   
\]
...
\[
   S = \sum_{n=0}^{+\infty} a_ncos(n\omega x) + b_nsin(n\omega x)    
\]
Ou encore sous forme exponentielle et pour une notation adaptée\footnote[2]{En fait, on a formellement \(S = \sum_{n = 0}^{+\infty} c_n e^{in\omega x} + c_{-n}e^{-in\omega x}\) qu'on simplifie avec la notation qui suit.}:
\[
   S = \sum_{n = -\infty}^{+\infty} c_n e^{in\omega x}     
\]
...
\[
   f(x)e^{-imx} = \sum_{-\infty}^{+\infty} c_n e^{ix(n - m)}
\]
L'idée étant alors de remarquer que si on intègre cette relation sur une période, on obtient:
\begin{align*}
   \int_{0}^{2\pi} f(x)e^{-imx} \d x &= \int_{0}^{2\pi} \sum_{-\infty}^{+\infty} c_n e^{ix(n - m)} \d x \\
   & = \sum_{-\infty}^{+\infty} c_n \int_{0}^{2\pi} e^{ix(n - m)} \d x\\
   & = 2\pi c_m
\end{align*}
En effet toutes les intégrales sont nulles pour \(n \neq m\), ce qui nous donne donc l'expression générale des coefficients de Fourier de \(f\):
\[
   c_n(f) = \frac{1}{2\pi} \int_{0}^{2\pi} f(x)e^{-inx} \d x
\]
On note souvent \(c_n(f)\) simplement \(c_n\) quand il n'y a pas de confusions possibles. Alors gràce aux formules d'Euler, on trouve alors les coefficients trigonométriques (plus pratiques à manipuler dans les cas simples) donnés pour tout \(n \neq 0\) par:
\[
   \begin{cases}
      a_n = \frac{1}{\pi} \int_{0}^{2\pi} f(x)\cos(nx) \d x\\
      b_n = \frac{1}{\pi} \int_{0}^{2\pi} f(x)\sin(nx) \d x\\
   \end{cases}   
\]
Avec le premier coefficient donné par \(c_0\), c'est la valeur moyenne de la fonction sur une période.
...
\begin{itemize}
   \item Si \(f\) est \textbf{paire} alors on a pour tout \(n\) que \(b_n = 0\) ou encore que \(c_n = c_{-n}\)
   \item Si \(f\) est \textbf{impaire} alors on a pour tout \(n\) que \(a_n = 0\) ou encore que \(c_n = -c_{-n}\).
\end{itemize}
...
\[
   \sum_{n = 0}^{+\infty} a_n\cos(nx) + b_n\sin(nx) = \sum_{n = -\infty}^{+\infty} c_ne^{inx}   
\]
...
Enfin, une formule fondamentale de la théorie des séries de Fourier est celle de \textbf{Parseval}, on définit d'abord le produit scalaire suivant sur l'espace des fonctions \(2\pi\)-périodiques:
\[
   \dotproduct{f}{g} := \int_{0}^{2\pi} f(t)\overline{g(t)} \d t   
\]
Alors on a l'égalité suivante:
\[
   \frac{1}{2\pi} \vectNorm{f}^2 = \frac{1}{2\pi} \int_{0}^{2\pi} |f(t)|^2 \d t = \sum_{n=-\infty}^{+\infty} |c_n|^2 = a_0^2 + \frac{1}{2} \sum_{n=1}^{+\infty} a_n^2 + b_n^2
\]
...
\subsection*{\subsecstyle{Exemples d'applications {:}}}
On considère la 
