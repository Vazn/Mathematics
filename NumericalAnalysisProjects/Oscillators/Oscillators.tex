...
\begin{document}
   ...
   \chapter{Systême masse-ressort}
      ...
      \end{itemize}
      On note alors \(y\) la fonction qui donne la position au temps \(t\) du centre de la masse. Pour modéliser le mouvement de la masse si on la sort de la position d'équilibre, on doit modéliser les forces en action.  D'aprés la 3em loi de Newton, on a la situation suivante:
      \begin{center}
         \begin{tikzpicture}[>=stealth]
            \path[pattern={Lines[angle=45,distance={8pt/sqrt(2)}]}] (-2,5) edge ++(4,0)
             rectangle ++ (4,1);
      ...
      On utilise alors un fait physique suivant appelée \textbf{loi des ressorts}:
      \begin{center}
         Pour allonger un ressort d'une longueur \(y_0\), la force nécessaire est \textbf{proportionelle} à \(y_0\), et la proportion est appelée \textbf{constante de raideur} du ressort.
      \end{center}
      On a donc un nouveau paramètre réel \(k\) qui caractérise la raideur du ressort, et l'accéleration que subit le ressort si on le lache aprés l'avoir allongé d'une longueur \(y_0\) est donnée par le \textbf{principe fondamental de la dynamique}, ie on a:
      \[
      ...
      \subsection{Champ de vecteur associé et portrait de phase}
         Si on considère l'équation différentielle initiale et une courbe \(\gamma(t) = (x(t), y(t))\) solution de cette équation, alors elle vérifie:
         \begin{center}
            blablabla
         \end{center}
         On peut alors représenter l'état de la masse à un instant \(t\) par sa position et sa vitesse, ie l'espace des phases est le plan repéré par \((y, y')\), et la dynamique est caractérisée par le champ de vecteur trouvé ci-dessus. Par exemple pour \(k = 1\), on a le champ de vecteur suivant:
         \begin{center}
            \begin{tikzpicture}
               \begin{axis}[
                  xmin = -2, xmax = 2,
...
\end{center}
   ...
   \chapter{Pendule simple}
      ...
      \]
      \subsection{Résolution}
      Malheureusement cette équation différentielle est trés difficile à résoudre analytiquement, le seul cas où la résolution est possible est pour \(\theta_0\) petit, alors on a:
      \[
         \sin(\theta) \sim \theta
      \]
      ...
      \begin{center}
         blablabla
      \end{center}
      On peut alors représenter l'état de la masse à un instant \(t\) par sa position et sa vitesse, ie l'espace des phases est le plan repéré par \((\theta, \theta')\), et la dynamique est caractérisée par le champ de vecteur trouvé ci-dessus. Par exemple\footnote[1]{Ici on utilise \(g=1\) pour faciliter la lisibilité du champ de vecteur.} pour \(g = 1\) et \(l = 1\), on a le champ de vecteur suivant:
      \begin{center}
         \begin{tikzpicture}
            \def\g{1}
            \def\ydom{3.5}
