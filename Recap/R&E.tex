...
\[
   \Bigr[ \P_{\alpha} \; \land \; \bigr( \P_k \implies \P_{k+1} \bigr) \Bigr] \implies \forall n \in \N \; ; \; \P_n   
\]
...
\[
   \Bigr[ \P_{\alpha} \; \land \; \P_{\alpha + 1} \; \land \; \bigr( \P_k  \land \P_{k-1}\implies \P_{k+1} \bigr) \Bigr] \implies \forall n \in \N \; ; \; \P_n   
\]

Enfin pour le cas limite, soit \(\Fam := (0, 1, 2, \ldots , n-1, n)\), alors si \(\P\) est de la forme \(\P_\F\), ie \(\P\) dépends de tout les rangs précédents, alors on peut utiliser une \textbf{récurrence forte} dont l'étape d'hérédité s'exprime:
\[
