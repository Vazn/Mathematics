...
\subsection*{\subsecstyle{Raisonnement par l'absurde {:}}}
...
\]
Qui nous donnera par l'absurde que \(\P\) est vraie. La contraposée est aussi utilisable en regroupant les \(n-1\) premiers termes.
...
\subsection*{\subsecstyle{Récurrences {:}}}
...
Admettons que \(\P(\alpha)\) soit vraie, supposons qu'elle soit vraie sur \(\inticc{\alpha}{k}\). Alors, on doit montrer que la propriété est vraie au rang \(k + 1\).\<

Alors, selon \textbf{le plus petit rang} nécessaire à compléter l'hérédité, on a:
\begin{align*}
    & \text{Si on a besoin de }\boxed{\P(k)} \text{ alors \textbf{on se ramène à une récurrence simple.}}\\
    & \text{Si on a besoin de }\boxed{\P(k-1)} \text{ alors \textbf{on se ramène à une récurrence double.}}\\
    & \text{Si on a besoin de }\boxed{\P(k-2)} \text{ alors \textbf{on se ramène à une récurrence triple.}}\\
    & \ldots\ldots\ldots
\end{align*}
Et donc, les initialisations et l'intervalle de notre hypothèse de récurrence changera en conséquence et on remarque alors que si le plus petit rang nécessaire est \(\P(k-p)\), alors on se ramène nécessairement à une \textbf{récurrence à \(p\) pas}, avec \(p\) initialisations et l'hypothèse de récurrence qui commence à \(\alpha + p\).\<
...
\subsection*{\subsecstyle{Propriétés de l'inclusion {:}}}
...
\subsection*{\subsecstyle{Propriétés du complémentaire et de la différence {:}}}
...
\]
\subsection*{\subsecstyle{Cardinalités {:}}}
...
\subsection*{\subsecstyle{Partition {:}}}
...
\chapter*{\chapterstyle{R\&E {:} Relations }}
...
\]
De ces propriétés on peut déduire les deux types fondamentaux de relation ci-dessous.
\pagebreak
...
Des relations d'ordre très connues sont la relation \(\boxed{\leq}\) sur les ensembles de nombres, ou encore la relation \(\boxed{\subseteq}\) sur l'ensemble des parties de \(E\).\<
...
Une \textbf{relation d'ordre} est une relation \textbf{réflexive, symétrique et transitive}. Intuitivement, elle met en relation les éléments des ensembles qui sont ``similaires``.\<

Des relations d'équivalence très connues sont la relation \(\boxed{=}\) et \(\boxed{\equiv}\) sur les ensembles de nombres, ou encore la relation \(\boxed{\sim}\) sur l'ensemble des suites.\<
...
\subsection*{\subsecstyle{Ensemble quotient {:}}}
...
On appelle \textbf{fonction et application} des cas particulier de relation entre deux ensembles.\<
...
On note \(D_f\) le sous-ensemble de \(E\) tel que \(f(x)\) existe, alors \(f\) est une \textbf{application} si et seulement si \(E = D_f\).\+
...
\]
Soit \(n \in \N\), on peut itérer la composition et on note \(f^{n}\) la composée n-ième de f avec elle même et on pose \(f^0 = Id_E\).
\subsection*{\subsecstyle{Restriction \& Prolongement {:}}}
...
Si il existe une bijection de \(E\) vers \(F\), alors on dit que ces ensembles sont \textbf{équipotents}, ce qui signifie qu'on a:
\[
    |E| = |F|
\]
